...
PWA 安裝狀態在「設定」頁顯示，`usePwaInstall.js` 會攔截 `beforeinstallprompt`，可安裝時會提供明確的安裝按鈕。手機版設定齒輪固定在左上角，避免底部 TabBar 或右側安全區擠壓。
...
- 手機版設定齒輪固定在左上角，標題自動讓位，避免右側安全區或工具列擠壓
...
1. **`window.confirm/alert` 換成自家 Toast/Modal** — 全 app UX 提升，散在 App.jsx、TransactionForm、SettingsView 等多處。
2. **TransactionList 分頁 / 滾動載入** — 動畫延遲上限已完成；下一步加 50 筆/頁或虛擬列表。
3. **`ReportView.jsx` 的 `computeRange` 收回去用 `utils/date.js`** — 維護債清理。
4. **搜尋條件儲存 + 釘選** — 重複查詢場景超常見。
5. **自動雲端備份提醒** — 超過 7 天沒同步在頂列顯示橘點。
6. **鍵盤快捷鍵** — N 新增、`/` 搜尋、Esc 取消、1–4 切分頁。
7. **長按交易 → 多選 → 批次刪除/改分類** — 大量誤分類時救命。

### 🟡 待做：中等投資（1–2 天）

8. **儲蓄目標 / 旅遊基金** — 目標金額 + 期限 + Dashboard 進度條，給「為什麼要記帳」一個情緒鉤子。
9. **分類改名 + 合併** — 已有 `migrations.js` 框架，套同樣手法做運行時的 rename（含同步 transactions）。
10. **每筆交易附收據圖** — IndexedDB（LocalStorage 5MB 上限），不要碰 base64 直接塞 LocalStorage。
11. **匯出 PDF / 分享圖卡** — `html2canvas` 把月報告變一張可分享圖。

### 🟠 待做：較大改動（2–4 天）

12. **帳戶 / 錢包系統** — 現金 / 街口 / 信用卡 / 悠遊卡，含轉帳。會動到 Transaction schema、Dashboard、ReportView，但讓 app 從「日記」進化成「資產管理」的分水嶺。
13. **多幣別 + 即時匯率** — `exchangerate.host` 免費 API，快取 24h。
14. **訂閱 / 固定支出自動偵測** — 固定交易儀表板已完成；下一步可從同分類規律支出自動偵測「你每月被扣 $XXX」。
15. **OCR 拍發票** — Tesseract.js 中文版，只抓金額 + 商家。中文發票辨識率不穩，當試驗品。
